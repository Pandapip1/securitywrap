...
The resulting wrapper should be given the appropriate capabilities, ownership, and/or permissions. The wrapper **is not portable**: it relies on `WRAP_EXECUTABLE` being available at the path specified at compile time, and on the specified group(ID)(s) / user(ID)(s) existing on the system.
